...
Hydra overrides work everywhere: `train.batch_size=8 train.grad_accum=2`, `model.lora_r=64 model.lora_alpha=64 model.lora_adaln_r=16 model.lora_dropout=0.05 'model.lora_targets=[attn.qkv_proj,attn.out_proj]'`, `EVALUATION.num_trials=20`. Every `train.probe_every` steps a fixed batch with fixed σ/noise seeds is scored without gradients (`probe/loss_video`, `probe/loss_action`, `probe/loss_video_full_noise`): a deterministic learning curve free of σ-sampling noise. Training also logs `train/grad_norm` (pre-clip), `train/grad_var` (trace of the per-micro-batch gradient covariance) and `train/grad_noise_scale` (B_simple: the batch size at which gradient noise equals signal; single-process runs only).
